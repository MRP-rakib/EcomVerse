...
function Cart() {
  ...
  const totalDiscount = product.reduce((acc, item) => {
    const price = Number(item.price) || 0;
  ...
  const Shipping = 15
  const totalPrice = ((subtotal - totalDiscount)+Shipping).toFixed(1)
    return (
                                    ...
                                    <tr key={product._id} className='shadow-[0_1px_13px_0] shadow-[rgba(0,0,0,0.05)]'>
                                        <td className='px-5 py-3 lg:px-10 lg:py-6 flex items-center gap-5'>
                                            <figure className='relative w-[30px] h-[20px] lg:w-[50px] lg:h-[40px] gap-5 overflow-hidden object-cover'>
                                                ...
                                                <span onClick={()=>handelDeleteItem(product.id)} className='flex items-center justify-center cursor-pointer absolute z-[1] w-3 h-3 rounded-full bg-[#DB4444] top-0 text-white'><IoIosClose /></span>
                                            </figure>
                                            <p className='text-[14] lg:text-[16px] text-nowrap line-clamp-1'>{product.title.slice(0, 35)}...</p>
                                        </td>
                                        <td className='text-[14] lg:text-[16px] px-5 py-3 lg:px-10 lg:py-6'>${product.price}</td>
                                        <td className='text-[14] lg:text-[16px] px-5 py-3 lg:px-10 lg:py-6'>
                                            <span className='py-[3px] px-2 lg:py-[6px] lg:px-3 border border-[rgba(0,0,0,0.4)] inline-flex items-center gap-4'>
                                                ...
                                                <span>
                                                    <span
                                                        onClick={() => handelIncreaseItem(product.id)}
                                                        className=' cursor-pointer' >
                                                        <FaAngleUp />
                                                    </span>
                                                    <span
                                                        onClick={() => handelDecreaseItem(product.id)}
                                                        className=' cursor-pointer' >
                                                        <FaAngleDown />
                                                    </span>
                                                </span>
                                        ...
                                        <td className='text-[14] lg:text-[16px] px-5 py-3 lg:px-10 lg:py-6'>${(product.price) * (product.quantity)}</td>
                                    </tr>))}
            ...
            <div className='pt-[80px] gap-8 flex-col lg:flex-row lg:gap-0 flex justify-between items-start'>
                ...
                <div className='w-full lg:w-[470px] py-8 px-6 border-2 rounded-[4px]'>
                    ...
                    <span className='flex justify-center pt-4'>
                        <Link to="#" className="cursor-pointer py-2 px-5 lg:py-3 lg:px-12 bg-[#DB4444] text-[14px] lg:text-[16px] font-medium rounded-[4px] text-[#FAFAFA]">
                            Procees to checkout
                        </Link>
                    </span>
                </div>
